...
void query1_UI(char *ID, void *dbDrivers, void *dbUsers, void *dbStats, void *dbRides){
    ...
    if(isdigit(ID[0])){
        ...
        if(isDriver(dbDrivers,id))
        {
            if (driver_get_account_status(dbDrivers,id) == 'a' && driver_get_trips(dbDrivers,id) != 0)
            {
                mvprintw(25, 27,"Nome                   Genero                         Idade                   Avaliaçao Media                 Numero de viagens               Total Auferido");                                   
        ...
        if(isUser(dbUsers,ID)){
            ...
            if(user_get_account_status(dbUsers,ID) == 'a' && user_get_trips(dbUsers,ID) != 0){
                mvprintw(25, 27,"Nome                   Genero                         Idade                   Avaliaçao Media                 Numero de viagens               Total gasto");                   
...
void query3_UI(char *Num, void *dbUsers,void *dbRides, void *dbDrivers, void *dbStats){
    ...
    while (!done) {
        ...
        mvprintw(45, xMax/2 - strlen ("Page 1 / 1")/2 , "Page %d / %d", page + 1, pages);
        int a = 0;
        mvprintw(9, 23,"Username                                                           Nome                                                       Distancia Total                         ");
...
void query7(char * N, char * cidade, void *dbStats, void *dbDrivers, short i){
    ...
    for(int i = 0; i < n;i++){
        ...
        int idm = city_get_info_id(dbStats,cidade,num_drivers);
        if(driver_get_account_status(dbDrivers,idm) != 'a'){
            n++;
        }
        else{
            ...
            fprintf(resultado,"%012d;%s;%.3f\n",idm,driver_get_name(dbDrivers,idm,driver,&size_d),aval_m);
        }
    }
    free(driver);
    free(id);
...
void query8_UI(char* gender, char* X, void *dbStats, void *dbRides, void *dbDrivers, void *dbUsers){
    short x = atoi(X);
    int yMax, xMax;
    int n = 0;
    getmaxyx(stdscr,yMax,xMax);
